...
template<typename T>
__global__
void hermitian2complex_kernel(const size_t problem_size, const size_t hermitian_size, const size_t size_y, const size_t size_z, T *input, size_t input_distance, T *output, size_t output_distance)
{
    size_t tid = hipBlockIdx_x * hipBlockDim_x + hipThreadIdx_x;
    ...
    size_t output_offset1 = output_offset + hipBlockIdx_y * problem_size;//notice for 1D, hipBlockIdx_y == 0 and thus has no effect for output_offset

    size_t y1 = hipBlockIdx_y % size_y;
    size_t z1 = hipBlockIdx_y / size_y;
    size_t y2 = y1 == 0 ? 0 : size_y - y1;
    size_t z2 = z1 == 0 ? 0 : size_z - z1;
    size_t output_offset2 = output_offset + (z2 * size_y + y2) * problem_size;

    T *output1 = output + output_offset1;
    T *output2 = output + output_offset2;
    ...
    if( tid < hermitian_size) //only read the first [N/2+1] elements due to conjugate symmetry, where N = problem_size
    {
	    ...
	    T res = input[tid];
        output1[tid] = res;
        output2[problem_size-tid] = T(res.x, -res.y);
...
void hermitian2complex(const void *data_p, void *back_p)
{
    ...
    const size_t size_y = data->node->length.size() > 1 ? data->node->length[1] : 1;
    const size_t size_z = data->node->length.size() > 2 ? data->node->length[2] : 1;
    if(precision == rocfft_precision_single)
        hipLaunchKernelGGL( hermitian2complex_kernel<float2>, grid, threads, 0, rocfft_stream, problem_size, hermitian_size, size_y, size_z, (float2 *)input_buffer, input_distance, (float2 *)output_buffer, output_distance);
    else
        hipLaunchKernelGGL( hermitian2complex_kernel<double2>, grid, threads, 0, rocfft_stream, problem_size, hermitian_size, size_y, size_z, (double2 *)input_buffer, input_distance, (double2 *)output_buffer, output_distance);
